...
export default function TeamsSection() {
  return (
    <section id="teams" className="relative py-24 px-6 bg-background overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-8 relative z-10">
        {[
          ...
          <motion.div
            key={idx}
            className="bg-card border border-border-default p-8 rounded-3xl relative overflow-hidden group"
          ...
          >
            ...
            <div className="w-14 h-14 bg-surface border border-border-subtle rounded-2xl flex items-center justify-center text-primary mb-6 shadow-inner">
              {item.icon}
            </div>
      ...
      <motion.div
        className="max-w-3xl mx-auto mt-16 p-6 bg-card border border-border-default rounded-full flex items-center justify-between"
      ...
      >
        <div className="flex items-center">
          <div className="flex -space-x-4 mr-6">
            {[1, 2, 3, 4, 5].map(num => (
              <img key={num} src={`https://i.pravatar.cc/100?img=${num * 10}`} alt="Team member" className="w-12 h-12 rounded-full border-4 border-card" />
            ))}
            <div className="w-12 h-12 rounded-full border-4 border-card bg-surface flex items-center justify-center text-text-primary font-medium text-xs z-10">+12</div>
          </div>
          <div className="hidden md:block">
            <h4 className="text-text-primary font-medium">Join 10,000+ teams</h4>
            <p className="text-xs text-text-muted">Collaborating globally</p>
          </div>
        </div>
        <button className="bg-primary text-white px-6 py-2.5 rounded-full font-semibold text-sm hover:bg-primary-hover transition-colors shadow-lg">
          Invite Your Team
        </button>
      </motion.div>
    </section>
